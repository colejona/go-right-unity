...
- **All work happens on a feature branch.** Never commit directly to main. Create a branch, do the work, open a PR, and wait for the user to approve and merge it. Before opening or updating a PR, always run `git status` to check for unstaged changes — Unity file deletions in particular require explicit `git rm` to be staged.
